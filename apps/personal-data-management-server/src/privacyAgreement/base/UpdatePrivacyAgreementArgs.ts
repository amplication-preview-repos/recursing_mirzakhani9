/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PrivacyAgreementWhereUniqueInput } from "./PrivacyAgreementWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { PrivacyAgreementUpdateInput } from "./PrivacyAgreementUpdateInput";

@ArgsType()
class UpdatePrivacyAgreementArgs {
  @ApiProperty({
    required: true,
    type: () => PrivacyAgreementWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => PrivacyAgreementWhereUniqueInput)
  @Field(() => PrivacyAgreementWhereUniqueInput, { nullable: false })
  where!: PrivacyAgreementWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => PrivacyAgreementUpdateInput,
  })
  @ValidateNested()
  @Type(() => PrivacyAgreementUpdateInput)
  @Field(() => PrivacyAgreementUpdateInput, { nullable: false })
  data!: PrivacyAgreementUpdateInput;
}

export { UpdatePrivacyAgreementArgs as UpdatePrivacyAgreementArgs };
